"""Shared test fixtures for erpclaw-region-eu tests."""
import json
import os
import sqlite3
import sys
...
def tmp_db(tmp_path):
    ...
    # Create minimal tables needed for EU skill tests
    conn.executescript("""
...
@pytest.fixture
def eu_company(tmp_db):
    """Create a German (DE) test company and return (db_path, company_id)."""
    conn = sqlite3.connect(tmp_db)
    conn.row_factory = sqlite3.Row
    company_id = str(uuid.uuid4())
    conn.execute(
        "INSERT INTO company (id, name, country) VALUES (?, ?, ?)",
        (company_id, "Deutsche Handels GmbH", "DE"),
    )
    conn.commit()
    conn.close()
...
@pytest.fixture
def fr_company(tmp_db):
    """Create a French test company and return (db_path, company_id)."""
    conn = sqlite3.connect(tmp_db)
    conn.row_factory = sqlite3.Row
    company_id = str(uuid.uuid4())
    conn.execute(
        "INSERT INTO company (id, name, country) VALUES (?, ?, ?)",
        (company_id, "Commerce France SARL", "FR"),
    )
    conn.commit()
    conn.close()
    return tmp_db, company_id

"""Shared test fixtures for erpclaw-region-ca tests."""
import json
import os
import sqlite3
import sys
...
def tmp_db(tmp_path):
    ...
    # Create minimal tables needed for Canada skill tests
    conn.executescript("""
...
@pytest.fixture
def ca_company(tmp_db):
    """Create a Canadian test company and return (db_path, company_id)."""
    conn = sqlite3.connect(tmp_db)
    conn.row_factory = sqlite3.Row
    company_id = str(uuid.uuid4())
    conn.execute(
        "INSERT INTO company (id, name, country, province) VALUES (?, ?, ?, ?)",
        (company_id, "Maple Leaf Industries Ltd", "CA", "ON"),
    )
    conn.commit()
    conn.close()
...
@pytest.fixture
def qc_company(tmp_db):
    """Create a Quebec test company and return (db_path, company_id)."""
    conn = sqlite3.connect(tmp_db)
    conn.row_factory = sqlite3.Row
    company_id = str(uuid.uuid4())
    conn.execute(
        "INSERT INTO company (id, name, country, province) VALUES (?, ?, ?, ?)",
        (company_id, "Quebec Industries Inc", "CA", "QC"),
    )
    conn.commit()
    conn.close()
    return tmp_db, company_id

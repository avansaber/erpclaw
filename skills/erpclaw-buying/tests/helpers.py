"""Shared test helpers for erpclaw-buying tests.
...
def create_test_naming_series(conn, company_id):
    """Create naming series for buying document types."""
...
def create_test_supplier(conn, company_id, name="Supplier Corp",
                         supplier_type="company", is_1099_vendor=0):
    """Create a supplier via the add-supplier action. Returns supplier_id."""
    ...
    if is_1099_vendor:
        kwargs["is_1099_vendor"] = str(is_1099_vendor)
    result = _call_action(ACTIONS["add-supplier"], conn, **kwargs)
    assert result["status"] == "ok", f"create_test_supplier failed: {result}"
    return result["supplier_id"]


def create_test_tax_template(conn, company_id, name="Purchase Tax 8%",
                             tax_type="purchase"):
    """Insert a test tax template with a single line. Returns (template_id, tax_account_id)."""
    ...
    tax_account_id = create_test_account(
        conn, company_id, "Input Tax", "asset",
        account_type="tax", account_number="1500",
    )
...
def setup_buying_environment(conn):
    """Create a complete environment for buying tests.
    Returns dict with company_id, fy_id, supplier_id, item_id, warehouse_id,
    payable_id, expense_id, stock_in_hand_id, stock_received_id,
    cost_center_id, tax_template_id, tax_account_id.
    """
    ...
    expense_id = create_test_account(
        conn, company_id, "Purchase Expense", "expense",
